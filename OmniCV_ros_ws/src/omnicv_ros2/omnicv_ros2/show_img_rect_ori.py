...
class PicamReader(Node):
    ...
    def image_callback(self, msg):
        ...
        if self.frame is not None:
            ...
            self.frame = mapper.fisheye2equirect(self.frame, self.out_shape, aperture=183, dely=-9)
            cv2.imshow("picam360 with cuda 1", self.frame)
            self.frame = mapper.equirect2cubemap(self.frame, modif=True)
            self.frame = mapper.cubemap2equirect(self.frame, self.out_shape)
            self.frame = self.frame[0:self.out_shape[0]//2, :]
